...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-gray-900">全部FAQ</h2>
        <a href="{{ route('admin.faqs.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-[#FF008A] text-white rounded-lg text-sm font-semibold hover:bg-[#FF33A1] transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        <form method="GET" action="{{ route('admin.faqs.index') }}" class="flex flex-wrap gap-4 items-end">
            <div>
                ...
                <select name="product_id" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#FF008A]/30 focus:border-[#FF008A]">
                    <option value="">全部产品</option>
                    @foreach(\App\Models\Product::active()->ordered()->get() as $prod)
            ...
            <a href="{{ route('admin.faqs.index') }}" class="px-4 py-2 text-sm text-gray-500 hover:text-gray-700">重置</a>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($faqs as $faq)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $faq->id }}</td>
                            <td class="px-6 py-4 text-sm font-medium text-gray-900 max-w-xs truncate">{{ Str::limit($faq->question, 60) }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $faq->product?->name ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $faq->category?->name ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.faqs.edit', $faq) }}" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-[#FF008A] bg-pink-50 rounded-lg hover:bg-pink-100 transition-colors">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/>
                                        </svg>
                                        编辑
                                    </a>
                                    <form action="{{ route('admin.faqs.destroy', $faq) }}" method="POST" onsubmit="return confirm('确定要删除此FAQ吗？')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">暂无FAQ</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
