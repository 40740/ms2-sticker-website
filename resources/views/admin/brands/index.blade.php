...
@section('content')
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.brands.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-[#FF008A] text-white rounded-lg text-sm font-semibold hover:bg-[#FF33A1] transition-colors">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($brands as $brand)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                @if($brand->image)
                                    <img src="{{ Storage::disk('uploads')->url($brand->image) }}" alt="{{ $brand->name }}" class="h-10 w-16 rounded-lg object-contain">
                                @else
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">
                                @if($brand->link)
                                    <a href="{{ $brand->link }}" target="_blank" class="text-[#FF008A] hover:text-[#FF33A1] truncate block max-w-[150px]">{{ $brand->link }}</a>
                                @else
                                    -
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $brand->sort_order }}</td>
                            <td class="px-6 py-4">
                                @if($brand->is_active)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.brands.edit', $brand) }}" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-[#FF008A] bg-pink-50 rounded-lg hover:bg-pink-100 transition-colors">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/>
                                        </svg>
                                        编辑
                                    </a>
                                    <form action="{{ route('admin.brands.destroy', $brand) }}" method="POST" onsubmit="return confirm('确定删除？')">
                                        @csrf
                                        @method('DELETE')
