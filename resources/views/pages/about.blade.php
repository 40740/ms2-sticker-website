...
<div class="max-w-[1200px] mx-auto px-6">
    <x-breadcrumb :items="[['title' => 'MeisaiPrinting']]" />
</div>
...
<section class="py-12 bg-white">
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="grid grid-cols-1 tablet:grid-cols-2 gap-10 items-center">
            ...
            {{-- Text --}}
            <div>
                <h2 class="text-section font-bold mb-6" style="color: #0095F4;">{{ $aboutStoryTitle }}</h2>
                <div class="text-body leading-relaxed space-y-4">
                    {{ $aboutStoryContent ?? 'Founded over 24 years ago, MeisaiPrinting started with a simple mission: to make high-quality custom stickers accessible to businesses of all sizes. What began as a small workshop has grown into a state-of-the-art manufacturing facility, serving clients across the globe.' }}
...
{{-- ====================================================================
   4. OUR VALUES  (Text left, image right — alternate layout)
   ==================================================================== --}}
    ...
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="grid grid-cols-1 tablet:grid-cols-2 gap-10 items-center">
            ...
            <div class="order-2 tablet:order-1">
                <h2 class="text-section font-bold mb-6" style="color: #0095F4;">{{ $aboutValuesTitle }}</h2>
                <div class="text-body leading-relaxed mb-6">
                    {{ $aboutValuesContent ?? 'At MeisaiPrinting, our core values guide everything we do. They shape our decisions, define our culture, and ensure we consistently deliver the best for our clients.' }}
                </div>
                <ul class="space-y-3">
...
<section class="py-12 bg-white">
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="grid grid-cols-1 tablet:grid-cols-2 gap-10 items-center">
            ...
            {{-- Text --}}
            <div>
                <h2 class="text-section font-bold mb-6" style="color: #0095F4;">{{ $aboutVisionTitle }}</h2>
                <div class="text-body leading-relaxed space-y-4">
                    {{ $aboutVisionContent ?? 'Our vision is to be the global leader in custom sticker and label solutions, recognized for our unwavering commitment to quality, sustainability, and customer satisfaction.' }}
...
{{-- ====================================================================
   6. OUR MISSION  (Text left, image right — alternate)
   ==================================================================== --}}
...
<section class="py-12 bg-bg-form">
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="grid grid-cols-1 tablet:grid-cols-2 gap-10 items-center">
            {{-- Text --}}
            <div class="order-2 tablet:order-1">
                <h2 class="text-section font-bold mb-6" style="color: #0095F4;">{{ $aboutMissionTitle }}</h2>
                <div class="text-body leading-relaxed mb-6">
                    {{ $aboutMissionContent ?? 'Our mission is to empower businesses with high-quality, affordable custom stickers and labels, delivered with exceptional service and speed.' }}
                </div>
                <ul class="space-y-3">
                    <li class="flex items-start gap-3">
...
{{-- ====================================================================
   7. OUR IDENTITY  (Image left, text right)
   ==================================================================== --}}
...
<section class="py-12 bg-white">
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="grid grid-cols-1 tablet:grid-cols-2 gap-10 items-center">
            ...
            {{-- Text --}}
            <div>
                <h2 class="text-section font-bold mb-6" style="color: #0095F4;">{{ $aboutIdentityTitle }}</h2>
                <div class="text-body leading-relaxed space-y-4">
                    {{ $aboutIdentityContent ?? 'MeisaiPrinting is more than a manufacturer — we are a creative partner. Our identity is built on the intersection of craftsmanship, technology, and joy.' }}
...
{{-- ====================================================================
   8. FACTORY SECTION  (Admin Configurable: title, content, video)
   ==================================================================== --}}

<section class="py-16 bg-bg-form">
    <div class="max-w-[1200px] mx-auto px-6">
        <div class="text-center mb-10">
            <h2 class="section-heading mb-4">{{ $factoryTitle }}</h2>
            @if($factoryContent)
                @php
                ...
                <div class="text-body leading-relaxed max-w-3xl mx-auto space-y-4">
                    @foreach($factoryParagraphs as $paragraph)
                        <p>{{ trim($paragraph) }}</p>
                    @endforeach
                </div>
            @else
        ...
        <div class="max-w-4xl mx-auto">
            <div class="relative aspect-video bg-black/5 rounded-lg overflow-hidden shadow-medium">
                @if($factoryVideoEmbed ?? '')
                    {{-- YouTube embed (lazy-loaded iframe) --}}
                    <iframe src="{{ $factoryVideoEmbed }}"
                            class="w-full h-full"
                            frameborder="0"
            ...
            @foreach($teamMembers->take(4) as $member)
                <div class="bg-white rounded-lg overflow-hidden shadow-light hover:shadow-medium transition-all duration-300 text-center">
                    ...
                    <div class="aspect-square bg-bg-form overflow-hidden">
                        @if($member->avatar)
                            <img src="{{ Storage::disk('uploads')->url($member->avatar) }}"
                                 alt="{{ $member->name }}"
                    ...
                    {{-- Info --}}
                    <div class="p-5">
                        <h3 class="text-feature font-bold text-box-title mb-1">{{ $member->name }}</h3>
                        <p class="text-accent text-sm font-semibold mb-3">{{ $member->title }}</p>
                        @if($member->bio)
                            <p class="text-body text-sm leading-relaxed line-clamp-3">{{ $member->bio }}</p>
                        @endif
                    </div>
                </div>
            @endforeach
