@props(['transparentOnTop' => true])

<header class="w-full fixed top-0 left-0 z-50 transition-all duration-300"
        :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'bg-white shadow-light' : 'bg-transparent'"
    ...
    <div class="hidden tablet:block border-b transition-all duration-300"
         :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'border-gray-100 bg-white' : 'border-transparent bg-transparent'">
        <div class="max-w-[1200px] mx-auto px-6 py-2 flex items-center justify-between text-sm">
            <div class="flex items-center gap-4" :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'text-body' : 'text-white'">
                <a href="mailto:{{ \App\Models\Setting::get('contact_email', 'info@meisaiprinting.com') }}" class="flex items-center gap-1 hover:text-brand transition-all duration-300">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                    </svg>
                    <span>{{ \App\Models\Setting::get('contact_email', 'info@meisaiprinting.com') }}</span>
                </a>
                <a href="tel:{{ \App\Models\Setting::get('contact_phone', '+1-800-123-4567') }}" class="flex items-center gap-1 hover:text-brand transition-all duration-300">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                    <span>{{ \App\Models\Setting::get('contact_phone', '+1-800-123-4567') }}</span>
                </a>
            </div>
            <div class="flex items-center gap-3" :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'text-body' : 'text-white'">
                <a href="{{ \App\Models\Setting::get('social_facebook', '#') }}" class="hover:text-brand transition-all duration-300" aria-label="Facebook">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="{{ \App\Models\Setting::get('social_instagram', '#') }}" class="hover:text-brand transition-all duration-300" aria-label="Instagram">
                ...
                <a href="{{ \App\Models\Setting::get('social_youtube', '#') }}" class="hover:text-brand transition-all duration-300" aria-label="YouTube">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                </a>
                <a href="{{ \App\Models\Setting::get('social_tiktok', '#') }}" class="hover:text-brand transition-all duration-300" aria-label="TikTok">
    ...
    <nav class="max-w-[1200px] mx-auto px-6 py-4 flex items-center justify-between">
        {{-- Logo --}}
        <a href="/" class="flex items-center gap-2 shrink-0" aria-label="{{ \App\Models\Setting::get('site_name', 'MeisaiPrinting') }} Home">
            @php
                $siteLogo = \App\Models\Setting::get('site_logo', '/images/logo.png');
                $siteName = \App\Models\Setting::get('site_name', 'MeisaiPrinting');
            @endphp
            @if($siteLogo && $siteLogo !== '/images/logo.png')
                <img src="{{ $siteLogo }}" alt="{{ $siteName }}" class="h-10 w-auto transition-all duration-300"
                     onerror="this.onerror=null;this.src='/images/logo.png';">
            @else
                <img src="/images/logo.png" alt="{{ $siteName }}" class="h-10 w-auto transition-all duration-300"
                     onerror="this.style.display='none';this.nextElementSibling.style.display='block';">
                <span class="text-2xl font-bold transition-colors duration-300"
                      :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'text-brand' : 'text-white'"
                      style="display:none;">
                    {{ $siteName }}
                </span>
            @endif
        ...
        <div class="hidden tablet:flex items-center gap-8">
            ...
            <div class="relative" x-data="{ open: false, timer: null }"
                ...
                <button class="nav-link flex items-center gap-1 transition-colors duration-300"
                        :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'text-body' : 'text-white'">
                    Material
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="hidden tablet:flex items-center gap-4">
            ...
            <button @click="searchOpen = !searchOpen" class="transition-colors duration-300"
                    :class="scrolled || !{{ $transparentOnTop ? 'true' : 'false' }} ? 'text-body hover:text-brand' : 'text-white hover:text-brand-hover'"
                    aria-label="Search">
